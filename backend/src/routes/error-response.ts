...
function containsTechnicalEnglish(value: string) {
  return /[A-Za-z]{3,}/.test(value)
}

function getFieldLabel(path: ZodIssue['path']) {
...
function normalizeValidationIssueMessage(issue: ZodIssue) {
...
function normalizeValidationIssues(issues: ZodIssue[]) {
...
function normalizeRouteErrorMessage(rawMessage: string | undefined, fallbackMessage: string) {
...
export function sendValidationError(response: Response, message: string, issues: ZodIssue[]) {
...
export function sendOperationError(response: Response, error: unknown, fallbackMessage: string, status = 400) {
  response.status(status).json({
    message: normalizeRouteErrorMessage(error instanceof Error ? error.message : undefined, fallbackMessage),
  })
}

export function sendAuthError(response: Response, message: string, status = 401) {
  response.status(status).json({
    message: normalizeRouteErrorMessage(message, message),
  })
}

export function sendPermissionError(response: Response, message: string, status = 403) {
  response.status(status).json({
    message: normalizeRouteErrorMessage(message, message),
  })
}