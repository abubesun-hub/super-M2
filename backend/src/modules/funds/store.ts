export type FundAccountType = 'revenue' | 'capital'

export type FundMovementDirection = 'inflow' | 'outflow' | 'transfer'
...
export type FundMovementReferenceType =
  | 'customer-payment'
  | 'shift'
  | 'capital-transaction'
  | 'expense'
...
export type FundMovement = {
  ...
  referenceType: FundMovementReferenceType
  referenceId?: string
...
export type FundMovementCreateInput = {
  movementDate: string
  direction: FundMovementDirection
  amountIqd: number
  sourceFundAccountId?: string
...
export type CapitalTransactionUpdateInput = CapitalTransactionCreateInput

function createDefaultFundAccounts(): FundAccount[] {
  return [
    {
      id: 'fund-revenue',
...
const storedFundAccounts: FundAccount[] = createDefaultFundAccounts()

const storedFundMovements: FundMovement[] = []
...
function createMovementNo(sequence: number) {
  const now = new Date()
...
function resolveFinalCashAllocations(amountIqd: number) {
  ...
  const allocations: Array<{ fundAccountId: string; fundAccountName: string; amountIqd: number }> = []
...
function requireFundAccount(accountId: string, fieldLabel: string) {
...
function assertMovementShape(input: FundMovementCreateInput) {
...
export function findFundAccountById(accountId: string) {
  return storedFundAccounts.find((account) => account.id === accountId) ?? null
}

export function findFundAccountByCode(code: string) {
  const normalizedCode = code.trim().toLowerCase()
  return storedFundAccounts.find((account) => account.code.toLowerCase() === normalizedCode) ?? null
}

export function findFundMovementById(movementId: string) {
  return storedFundMovements.find((movement) => movement.id === movementId) ?? null
}

export function getCapitalContributorBalance(contributorName: string, excludedMovementId?: string) {
...
function reverseFundMovementBalanceEffect(movement: FundMovement) {
...
export function createFundMovement(input: FundMovementCreateInput) {
...
export function createFinalCashOutflow(input: Omit<FundMovementCreateInput, 'direction' | 'sourceFundAccountId'>) {
  if (!Number.isFinite(input.amountIqd) || input.amountIqd <= 0) {
    throw new Error('مبلغ حركة الصندوق يجب أن يكون أكبر من صفر.')
  }
...
export function createCapitalTransaction(input: CapitalTransactionCreateInput) {
...
export function deleteCapitalTransaction(movementId: string) {
...
export function updateCapitalTransaction(movementId: string, input: CapitalTransactionUpdateInput) {
