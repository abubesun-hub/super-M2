...
import type { EmployeeCompensationCreateInput, EmployeeCreateInput, EmployeeUpdateInput, EmployeeAbsenceUpdateInput } from '../modules/employees/schemas.js'
...
import { DEFAULT_ADMIN_PIN, DEFAULT_ADMIN_USERNAME, buildEmployeeCumulativePayrollSummary, buildMonthlyPayrollSummary, isEmployeeCompensationOutflow, resolveEmployeeCompensationInput, type Employee, type EmployeeAbsence, type EmployeeCompensation, type MonthlyPayrollSummary } from '../modules/employees/store.js'
import type { Expense, ExpenseCategory, ExpenseCreateRecordInput } from '../modules/expenses/store.js'
import type { CapitalTransactionCreateInput, CapitalTransactionUpdateInput, FundAccount, FundMovement, FundMovementCreateInput } from '../modules/funds/store.js'
import type { StoredPurchaseReceipt } from '../modules/purchases/store.js'
import type { CatalogProduct, InventoryBatch, ProductUpsertInput, StockMovement } from '../modules/products/store.js'
...
import { calculateSaleReturnValue, resolveSaleReturnSettlement } from '../modules/sales/store.js'
...
import { normalizeSystemSettings, type SystemSettings, type SystemSettingsUpdateInput } from '../modules/settings/store.js'
...
type SaleReturnRow = {
  ...
  invoice_id: string
  reason: string
  settlement_type: 'cash-refund' | 'deduct-customer-balance'
  cash_refund_iqd: string | number
  debt_relief_iqd: string | number
  created_at: Date | string
}
...
type InventoryBatchRow = {
  ...
  retail_unit_cost: string | number
  created_at: Date | string
}

type SaleItemBatchAllocationRow = {
  id: string
  sale_item_id: string
  product_id: string
  batch_id: string
...
type SupplierPaymentRow = {
  ...
  source_fund_account_id: string | null
  source_fund_account_name: string | null
  notes: string | null
  created_at: Date | string
}
...
type CustomerPaymentRow = {
  ...
  shift_id: string | null
  terminal_name: string | null
  destination_fund_account_id: string | null
  destination_fund_account_name: string | null
  notes: string | null
  created_at: Date | string
}
...
type FundAccountRow = {
  ...
  current_balance_iqd: string | number
  is_system: boolean
  is_active: boolean
  created_at: Date | string
}
...
type FundMovementRow = {
  ...
  reference_type: 'customer-payment' | 'shift' | 'capital-transaction' | 'expense' | 'supplier-payment'
...
type EmployeeRow = {
  ...
  name: string
  role: 'admin' | 'cashier' | 'inventory' | 'accountant'
  start_date: Date | string | null
  monthly_salary_iqd: string | number | null
  employment_status: 'active' | 'suspended' | 'terminated' | null
  service_end_date: Date | string | null
  payroll_type: 'monthly' | 'daily' | 'shift' | null
  payroll_rate_iqd: string | number | null
  pin_hash: string
  notes: string | null
  is_active: boolean
  created_at: Date | string
...
type EmployeeCompensationRow = {
  ...
  kind: 'salary' | 'payment' | 'advance' | 'bonus' | 'deduction'
  amount_iqd: string | number
  calculation_method: 'manual' | 'monthly'
...
type ShiftRow = {
  ...
  opening_float_iqd: string | number
  opening_note: string | null
  opened_at: Date | string
  closed_at: Date | string | null
  closing_note: string | null
  closing_cash_iqd: string | number | null
  remitted_to_fund_account_id: string | null
  remitted_to_fund_account_name: string | null
  remittance_movement_id: string | null
  invoices_count: number | null
  returns_count: number | null
  gross_sales_iqd: string | number | null
  returns_value_iqd: string | number | null
  net_sales_iqd: string | number | null
  invoice_collections_iqd: string | number | null
  customer_payments_count: number | null
  customer_payments_iqd: string | number | null
  collected_cash_iqd: string | number | null
  credit_sales_iqd: string | number | null
  expected_cash_iqd: string | number | null
  cash_difference_iqd: string | number | null
  status: 'open' | 'closed'
}
...
type ExpenseCategoryRow = {
  ...
  name: string
  code: string
  kind: 'operating' | 'service' | 'payroll' | 'supplier' | 'other'
...
type ExpenseRow = {
  ...
  category_kind: 'operating' | 'service' | 'payroll' | 'supplier' | 'other'
  amount_iqd: string | number
  ...
  source_fund_account_name: string | null
  shift_id: string | null
  reference_type: 'manual' | 'supplier-payment' | 'employee-compensation'
  reference_id: string | null
  status: 'posted'
  created_at: Date | string
}
...
type SystemSettingsRow = {
  id: string
  payload: SystemSettingsUpdateInput
...
function sortOperationalAccounts<T extends { code: string }>(accounts: T[]) {
  const weightByCode: Record<string, number> = { revenue: 0, capital: 1 }
  return [...accounts].sort((left, right) => (weightByCode[left.code] ?? 99) - (weightByCode[right.code] ?? 99))
}

function encodeFinalCashMetadata(notes: string | undefined, allocations: FinalCashAllocation[]) {
...
function parseFinalCashMetadata(notes: string | null | undefined) {
  if (!notes?.startsWith(FINAL_CASH_METADATA_PREFIX)) {
    return {
      publicNotes: notes ?? undefined,
      allocations: [] as FinalCashAllocation[],
    }
  }
  ...
  try {
    const allocations = JSON.parse(Buffer.from(encodedAllocations, 'base64url').toString('utf8')) as FinalCashAllocation[]
    return {
...
function mapInventoryBatchRow(row: InventoryBatchRow): InventoryBatch {
  return {
    id: row.id,
    productId: row.product_id,
    productName: row.product_name,
...
async function loadInventoryBatches(database: Queryable, productIds?: string[]): Promise<InventoryBatch[]> {
  const result = productIds && productIds.length > 0
    ? await database.query<InventoryBatchRow>(
        `
...
async function getLockedOpeningBatch(client: PoolClient, productId: string) {
  const result = await client.query<InventoryBatchRow>(
    `
      select id, product_id, product_name, source, purchase_receipt_item_id, batch_no, expiry_date, purchase_date, supplier_name, received_quantity, remaining_quantity, retail_unit_cost, created_at
      from app_inventory_batches
...
async function getPurchaseBatchRemaining(client: PoolClient, productId: string) {
  const result = await client.query<{ remaining_quantity: string | number | null }>(
    `
...
async function syncOpeningBatch(client: PoolClient, product: Pick<ProductRow, 'id' | 'name' | 'stock_qty' | 'purchase_price'>) {
...
async function loadConsumableBatches(client: PoolClient, productId: string) {
  const result = await client.query<InventoryBatchRow>(
    `
      select id, product_id, product_name, source, purchase_receipt_item_id, batch_no, expiry_date, purchase_date, supplier_name, received_quantity, remaining_quantity, retail_unit_cost, created_at
      from app_inventory_batches
...
async function consumeInventoryBatchesForSale(
  client: PoolClient,
  input: { saleItemId: string; productId: string; quantity: number },
) {
  ...
  for (const batch of batches) {
    ...
    await client.query(
      'update app_inventory_batches set remaining_quantity = $1 where id = $2',
      [roundQuantity(asNumber(batch.remaining_quantity) - consumedQuantity), batch.id],
    )
    await client.query(
      `
...
async function restoreInventoryBatchesForSaleReturn(
  client: PoolClient,
  input: { saleItemId: string; quantity: number },
) {
  const allocationsResult = await client.query<SaleItemBatchAllocationRow>(
    `
  ...
  for (const allocation of allocationsResult.rows) {
    ...
    await client.query(
      'update app_inventory_batches set remaining_quantity = remaining_quantity + $1 where id = $2',
      [restoredQuantity, allocation.batch_id],
    )
    await client.query(
      'update app_sale_item_batch_allocations set returned_quantity = $1 where id = $2',
      [roundQuantity(asNumber(allocation.returned_quantity) + restoredQuantity), allocation.id],
    )
...
function mapFundAccountRow(row: FundAccountRow): FundAccount {
  return {
    id: row.id,
    name: row.name,
    code: row.code,
...
function mapFundMovementRow(row: FundMovementRow): FundMovement {
...
function mapEmployeeCompensationRow(row: EmployeeCompensationRow): EmployeeCompensation {
...
function mapEmployeeAbsenceRow(row: EmployeeAbsenceRow): EmployeeAbsence {
  return {
    id: row.id,
    employeeId: row.employee_id,
    employeeName: row.employee_name,
...
function mapSystemSettingsRow(row: SystemSettingsRow): SystemSettings {
...
async function loadSystemSettings(database: Queryable): Promise<SystemSettings> {
  const result = await database.query<SystemSettingsRow>(
    `
...
async function getLockedEmployee(database: Queryable, employeeId: string) {
...
async function getLockedEmployeeAbsence(database: Queryable, employeeId: string, absenceId: string) {
  const result = await database.query<EmployeeAbsenceRow>(
    `
...
async function getLockedShift(database: Queryable, shiftId: string) {
...
async function getLockedExpenseCategory(database: Queryable, categoryId: string) {
  const result = await database.query<ExpenseCategoryRow>(
    `
...
async function getLockedExpenseCategoryByCode(database: Queryable, code: string) {
  const result = await database.query<ExpenseCategoryRow>(
    `
      select id, name, code, kind, description, is_system, is_active, created_at
      from app_expense_categories
...
async function getLockedFundAccount(database: Queryable, accountId: string) {
  const result = await database.query<FundAccountRow>(
    `
...
async function getLockedFundAccountByCode(database: Queryable, code: string) {
  const result = await database.query<FundAccountRow>(
    `
      select id, name, code, account_type, current_balance_iqd, is_system, is_active, created_at
      from app_fund_accounts
...
async function loadFundAccounts(database: Queryable): Promise<FundAccount[]> {
  const result = await database.query<FundAccountRow>(
    `
      select id, name, code, account_type, current_balance_iqd, is_system, is_active, created_at
      from app_fund_accounts
...
async function loadFundMovements(database: Queryable): Promise<FundMovement[]> {
  const result = await database.query<FundMovementRow>(
    `
...
async function loadFundMovementById(database: Queryable, movementId: string): Promise<FundMovement | null> {
  const result = await database.query<FundMovementRow>(
    `
      select id, movement_no, movement_date, direction, amount_iqd,
             source_fund_account_id, source_fund_account_name,
...
async function loadCapitalTransactions(database: Queryable): Promise<FundMovement[]> {
  const result = await database.query<FundMovementRow>(
    `
      select id, movement_no, movement_date, direction, amount_iqd,
             source_fund_account_id, source_fund_account_name,
...
async function getCapitalTransactionRowForUpdate(client: PoolClient, movementId: string) {
  const result = await client.query<FundMovementRow>(
    `
      select id, movement_no, movement_date, direction, amount_iqd,
             source_fund_account_id, source_fund_account_name,
             destination_fund_account_id, destination_fund_account_name,
...
async function getCapitalContributorBalance(client: PoolClient, contributorName: string, excludedMovementId?: string) {
  const result = excludedMovementId
    ? await client.query<{ balance_iqd: string | number | null }>(
        `
          select coalesce(sum(
            case
...
async function reverseFundMovementEntry(client: PoolClient, movement: FundMovementRow) {
  ...
  if (!movement.source_fund_account_id && movement.source_fund_account_name === FINAL_CASH_LABEL && allocations.length) {
    for (const allocation of allocations) {
      ...
      await client.query(
        'update app_fund_accounts set current_balance_iqd = $2 where id = $1',
        [sourceAccount.id, roundMoney(asNumber(sourceAccount.current_balance_iqd) + allocation.amountIqd)],
      )
    }
  ...
  if (movement.source_fund_account_id) {
    ...
    await client.query(
      'update app_fund_accounts set current_balance_iqd = $2 where id = $1',
      [sourceAccount.id, roundMoney(asNumber(sourceAccount.current_balance_iqd) + amountIqd)],
    )
  }
  ...
  if (movement.destination_fund_account_id) {
    ...
    await client.query(
      'update app_fund_accounts set current_balance_iqd = $2 where id = $1',
      [destinationAccount.id, roundMoney(asNumber(destinationAccount.current_balance_iqd) - amountIqd)],
    )
  }
}

async function resolveFinalCashAllocations(client: PoolClient, amountIqd: number) {
  ...
  ].filter((account): account is NonNullable<typeof account> => Boolean(account && account.is_active)))
  ...
  const allocations: FinalCashAllocation[] = []
...
async function createFinalCashOutflowEntry(client: PoolClient, input: Omit<FundMovementCreateInput, 'direction' | 'sourceFundAccountId'>) {
  ...
  for (const allocation of allocations) {
    ...
    await client.query(
      'update app_fund_accounts set current_balance_iqd = $2 where id = $1',
      [account.id, roundMoney(asNumber(account.current_balance_iqd) - allocation.amountIqd)],
    )
  }
...
async function loadShiftCustomerPayments(database: Queryable, shiftId: string): Promise<CustomerPayment[]> {
  const result = await database.query<CustomerPaymentRow>(
    `
      select id, payment_no, customer_id, customer_name, currency_code, exchange_rate, amount, amount_iqd,
             shift_id, terminal_name, destination_fund_account_id, destination_fund_account_name, notes, created_at
...
async function getOpenShiftByEmployee(database: Queryable, employeeId: string): Promise<ShiftRow | null> {
  const result = await database.query<ShiftRow>(
    `
      select id, shift_no, employee_id, employee_name, terminal_name, opening_float_iqd, opening_note, opened_at, closed_at, closing_note, status,
...
async function buildLiveShiftSummary(database: Queryable, shift: ShiftRow) {
  const invoiceIdsResult = await database.query<{ id: string }>(
    'select id from app_sale_invoices where shift_id = $1 order by created_at asc',
    [shift.id],
  )
...
async function generateEmployeeNo(client: PoolClient) {
...
function isProtectedAdminRow(employee: Pick<EmployeeRow, 'username' | 'role'>) {
  return employee.role === 'admin' && employee.username?.toLowerCase() === DEFAULT_ADMIN_USERNAME
}

export async function ensureDefaultAdminAccount(pool: Pool) {
  const adminId = 'emp-admin-default'
  const adminPinHash = hashPin(DEFAULT_ADMIN_PIN)
  const existingAdminResult = await pool.query<{ id: string }>(
    `
      select id
      from app_employees
...
async function getLockedPurchaseReceipt(client: PoolClient, receiptId: string) {
  ...
  const itemsResult = await client.query<PurchaseReceiptItemRow>(
    `
      select id, receipt_id, product_id, name, quantity, base_quantity, entry_unit, entry_unit_label, batch_no, expiry_date, unit_cost, unit_cost_iqd, line_total, line_total_iqd
      from app_purchase_receipt_items
      where receipt_id = $1
...
async function getLatestPurchaseBatchRetailCost(client: PoolClient, productId: string) {
  const result = await client.query<{ retail_unit_cost: string | number }>(
    `
...
async function applyPurchaseReceiptItems(
  client: PoolClient,
  input: CreatePurchaseReceiptInput,
  supplierName: string | undefined,
  purchaseDateLabel: string,
  supplierInvoiceLabel: string | undefined,
) {
  const normalizedItems: Array<StoredPurchaseReceipt['items'][number] & { purchaseItemId: string }> = []
...
async function reversePurchaseReceiptInventory(
  client: PoolClient,
  receipt: PurchaseReceiptRow & { items: PurchaseReceiptItemRow[] },
  actionLabel: string,
) {
  const itemIds = receipt.items.map((item) => item.id)
  const batchesResult = itemIds.length > 0
    ? await client.query<InventoryBatchRow>(
        `
          select id, product_id, product_name, source, purchase_receipt_item_id, batch_no, expiry_date, purchase_date, supplier_name, received_quantity, remaining_quantity, retail_unit_cost, created_at
          from app_inventory_batches
    ...
    : { rows: [] as InventoryBatchRow[] }

  const batchByItemId = new Map<string, InventoryBatchRow>()
...
async function generateEmployeeCompensationNo(client: PoolClient) {
  const today = new Date().toISOString().slice(0, 10)
  const sequenceResult = await client.query<{ value: number }>(
    `
      insert into app_daily_sequences (seq_key, seq_date, value)
...
async function generateShiftNo(client: PoolClient) {
  const today = new Date().toISOString().slice(0, 10)
  const sequenceResult = await client.query<{ value: number }>(
    `
      insert into app_daily_sequences (seq_key, seq_date, value)
...
async function generateExpenseNo(client: PoolClient) {
  const today = new Date().toISOString().slice(0, 10)
  const sequenceResult = await client.query<{ value: number }>(
    `
      insert into app_daily_sequences (seq_key, seq_date, value)
...
async function generateFundMovementNo(client: PoolClient) {
  const today = new Date().toISOString().slice(0, 10)
  const sequenceResult = await client.query<{ value: number }>(
    `
      insert into app_daily_sequences (seq_key, seq_date, value)
...
export function createPostgresDataAccess(pool: Pool): DataAccess {
  return {
    ...
    purchases: {
      ...
      async createReceipt(input: CreatePurchaseReceiptInput) {
        ...
        try {
          ...
          const normalizedItems = await applyPurchaseReceiptItems(
            client,
            input,
            supplierName,
            purchaseDateLabel,
            supplierInvoiceLabel || undefined,
          )
      ...
      async updateReceipt(receiptId: string, input: CreatePurchaseReceiptInput) {
        ...
        try {
          ...
          if (oldSupplier) {
            await client.query(
              'update app_suppliers set current_balance = current_balance - $1 where id = $2',
              [asNumber(existingReceipt.total_cost_iqd), oldSupplier.id],
            )
          }
        ...
        try {
          ...
          if (existingReceipt.supplier_id) {
            ...
            if (supplier) {
              await client.query(
                'update app_suppliers set current_balance = current_balance - $1 where id = $2',
                [asNumber(existingReceipt.total_cost_iqd), supplier.id],
              )
            }
          }
    ...
    customers: {
      ...
      async createPayment(customerId, input: CustomerPaymentInput & { createdByEmployeeId: string; createdByEmployeeName: string }) {
        const client = await pool.connect()
    ...
    employees: {
      ...
      async listCompensations(employeeId: string) {
        const employee = await pool.query<{ id: string }>('select id from app_employees where id = $1', [employeeId])

        if (!employee.rows[0]) {
          throw new Error('الموظف المطلوب غير موجود.')
        }

        const result = await pool.query<EmployeeCompensationRow>(
          `
      ...
      async listAbsences(employeeId: string) {
        ...
        const result = await pool.query<EmployeeAbsenceRow>(
          `
            select id, employee_id, employee_name, absence_date, deduction_days, notes, created_by_employee_id, created_by_employee_name, created_at
            from app_employee_absences
            where employee_id = $1
      ...
      async listMonthlyPayroll(month: string) {
        const employeesResult = await pool.query<EmployeeRow>(
          `
            select id, employee_no, username, name, role, start_date, monthly_salary_iqd, employment_status, service_end_date, payroll_type, payroll_rate_iqd, pin_hash, notes, is_active, created_at
            from app_employees
            order by employee_no asc
        ...
        const compensationsResult = await pool.query<EmployeeCompensationRow>(
          `
            select id, payment_no, employee_id, employee_name, kind, amount_iqd, calculation_method, payment_method, payment_date,
                   period_label, notes, created_by_employee_id, created_by_employee_name, created_at
            from app_employee_compensations
        ...
        const absencesResult = await pool.query<EmployeeAbsenceRow>(
          `
            select id, employee_id, employee_name, absence_date, deduction_days, notes, created_by_employee_id, created_by_employee_name, created_at
            from app_employee_absences
        ...
        return employeesResult.rows
          ...
          .map((employee): MonthlyPayrollSummary => buildMonthlyPayrollSummary(
            employee,
            compensationsResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeCompensationRow),
            absencesResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeAbsenceRow),
            month,
          ))
      },
      async listCumulativePayroll(throughMonth: string) {
        const employeesResult = await pool.query<EmployeeRow>(
          `
            select id, employee_no, username, name, role, start_date, monthly_salary_iqd, employment_status, service_end_date, payroll_type, payroll_rate_iqd, pin_hash, notes, is_active, created_at
            from app_employees
        ...
        return employees.map((employee) => buildEmployeeCumulativePayrollSummary(
          employee,
          compensationsResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeCompensationRow),
          absencesResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeAbsenceRow),
          throughMonth,
      ...
      async createCompensation(employeeId: string, input: EmployeeCompensationCreateInput & { createdByEmployeeId: string; createdByEmployeeName: string }) {
        const client = await pool.connect()
      ...
      async createAbsence(employeeId: string, input) {
        const client = await pool.connect()
      ...
      async updateAbsence(employeeId: string, absenceId: string, input: EmployeeAbsenceUpdateInput & { createdByEmployeeId: string; createdByEmployeeName: string }) {
        const client = await pool.connect()
      ...
      async deleteAbsence(employeeId: string, absenceId: string) {
        const client = await pool.connect()
      ...
      async settleMonthlyPayroll(input) {
        ...
        try {
          ...
          const compensationsResult = await client.query<EmployeeCompensationRow>(
            `
              select id, payment_no, employee_id, employee_name, kind, amount_iqd, calculation_method, payment_method, payment_date,
                     period_label, notes, created_by_employee_id, created_by_employee_name, created_at
              from app_employee_compensations
          ...
          const absencesResult = await client.query<EmployeeAbsenceRow>(
            `
              select id, employee_id, employee_name, absence_date, deduction_days, notes, created_by_employee_id, created_by_employee_name, created_at
              from app_employee_absences
              where employee_id = any($1::text[]) and to_char(absence_date, 'YYYY-MM') = $2
          ...
          for (const employee of selectedEmployees) {
            const summary = buildMonthlyPayrollSummary(
              employee,
              compensationsResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeCompensationRow),
              absencesResult.rows.filter((row) => row.employee_id === employee.id).map(mapEmployeeAbsenceRow),
              input.month,
      ...
      async updateEmployee(employeeId: string, input: EmployeeUpdateInput) {
        try {
          const currentResult = await pool.query<EmployeeRow>(
            `
              select id, employee_no, username, name, role, start_date, monthly_salary_iqd, employment_status, service_end_date, payroll_type, payroll_rate_iqd, pin_hash, notes, is_active, created_at
              from app_employees
              where id = $1
      ...
      async authenticate(login: string, pin: string) {
      ...
      async resetPin(employeeId: string, pin: string) {
        try {
          const result = await pool.query<EmployeeRow>(
            `
              update app_employees
    ...
    shifts: {
      async listShifts(employeeId?: string) {
        const result = employeeId
          ? await pool.query<ShiftRow>(
              `
                select id, shift_no, employee_id, employee_name, terminal_name, opening_float_iqd, opening_note, opened_at, closed_at, closing_note, status
      ...
      async createShift(input) {
        ...
        try {
          ...
          const openShiftCheck = await client.query<{ id: string }>(
            'select id from app_cashier_shifts where employee_id = $1 and status = $2 for update',
            [employee.id, 'open'],
          )
      ...
      async closeShift(shiftId, input) {
        ...
        try {
          ...
          const invoiceIdsResult = await client.query<{ id: string }>(
            'select id from app_sale_invoices where shift_id = $1 order by created_at asc',
            [shiftId],
          )
          const invoices = invoiceIdsResult.rows.length > 0
    ...
    expenses: {
      ...
      async createCategory(input: ExpenseCategoryCreateInput) {
      ...
      async createExpense(input: ExpenseCreateRecordInput) {
        const client = await pool.connect()
    ...
    sales: {
      ...
      async createInvoice(input: CreateSaleInvoiceInput) {
        ...
        try {
          ...
          const aggregatedByProduct = new Map<string, { name: string; quantity: number }>()
      ...
      async createReturn(invoiceId: string, input: CreateSaleReturnInput) {
        ...
        try {
          ...
          await client.query(
            'insert into app_sale_returns (id, invoice_id, reason, settlement_type, cash_refund_iqd, debt_relief_iqd) values ($1, $2, $3, $4, $5, $6)',
            [saleReturnId, invoiceId, input.reason, input.settlementType, settlement.cashRefundIqd, settlement.debtReliefIqd],
          )
          ...
          if (invoice.customerId && settlement.debtReliefIqd > 0) {
            ...
            await client.query(
              'update app_customers set current_balance = $1 where id = $2',
              [roundMoney(asNumber(customer.current_balance) - settlement.debtReliefIqd), customer.id],
            )
          }
    ...
    funds: {
      ...
      async deleteCapitalTransaction(movementId: string) {
        const client = await pool.connect()
    ...
    settings: {
      ...
      async updateSettings(input: SystemSettingsUpdateInput) {
        const normalized = normalizeSystemSettings({
          ...input,
          updatedAt: new Date().toISOString(),
        })

        const result = await pool.query<SystemSettingsRow>(
          `
