...
export async function seedPostgresDefaults(target: Pick<Pool, 'query'>) {
  ...
  const existingSuppliers = await target.query<{ count: string }>('select count(*)::text as count from app_suppliers')
  ...
  const existingExpenseCategories = await target.query<{ count: string }>('select count(*)::text as count from app_expense_categories')
  ...
  const existingFundAccounts = await target.query<{ count: string }>('select count(*)::text as count from app_fund_accounts')
