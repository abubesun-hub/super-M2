import { adjustCustomerBalance, createCustomer, createCustomerPayment, deleteCustomer, findCustomerById, listCustomerPayments, listCustomers, resetCustomersStore, updateCustomer, } from '../modules/customers/store.js';
import { authenticateEmployee, buildEmployeeCumulativePayrollSummary, buildMonthlyPayrollSummary, createEmployeeAbsence, createEmployeeCompensation, createEmployee, deleteEmployeeAbsence, findEmployeeById, isEmployeeCompensationOutflow, listEmployeeAbsences, listEmployeeCompensations, listEmployees, resetEmployeesStore, resetEmployeePin, setEmployeeActive, settleMonthlyPayroll, updateEmployeeAbsence, updateEmployee, } from '../modules/employees/store.js';
...
import { adjustProductStock, createCatalogProduct, deleteCatalogProduct, listInventoryBatches, listCatalogProducts, listStockMovements, resetProductsStore, reversePurchaseFromInventory, receivePurchaseToInventory, restoreSaleToInventory, updateCatalogProduct, applySaleToInventory, } from '../modules/products/store.js';
import { createPurchaseReceipt, deletePurchaseReceipt, findPurchaseReceiptById, listPurchaseReceipts, resetPurchasesStore, updatePurchaseReceipt, } from '../modules/purchases/store.js';
import { createSaleInvoice, createSaleReturn, findSaleInvoiceById, getReturnedQuantity, listSaleInvoices, resetSalesStore, resolveSaleReturnSettlement, } from '../modules/sales/store.js';
import { adjustSupplierBalance, createSupplier, createSupplierPayment, deleteSupplier, findSupplierById, listSupplierPayments, listSuppliers, resetSuppliersStore, updateSupplier, } from '../modules/suppliers/store.js';
import { getFinalCashBalanceIqd } from '../modules/funds/store.js';
import { createExpenseCategory, createExpenseRecord, findExpenseCategoryByCode, listExpenseCategories, listExpenses, resetExpensesStore, } from '../modules/expenses/store.js';
import { createCapitalTransaction, createFundMovement, deleteCapitalTransaction, findFundAccountByCode, findFundAccountById, listCapitalTransactions, listFundAccounts, listFundMovements, resetFundsStore, updateCapitalTransaction, } from '../modules/funds/store.js';
...
function normalizePurchaseReceiptItems(input) {
    ...
    return {
        ...
        sales: {
            ...
            async createInvoice(input) {
                ...
                applySaleToInventory(storedItems.map((item) => ({
                    saleItemId: item.id,
        ...
        purchases: {
            ...
            async updateReceipt(receiptId, input) {
                ...
                reversePurchaseFromInventory(existingReceipt.items.map((item) => ({
                    receiptItemId: item.receiptItemId,
                    productId: item.productId,
                    name: item.name,
                    retailQuantity: item.baseQuantity,
                })), `عكس سند الشراء ${existingReceipt.receiptNo} قبل التعديل`, 'تعديل');
                if (oldSupplier) {
            ...
            async deleteReceipt(receiptId) {
                ...
                reversePurchaseFromInventory(existingReceipt.items.map((item) => ({
                    ...
                    name: item.name,
                    retailQuantity: item.baseQuantity,
                })), `حذف سند الشراء ${existingReceipt.receiptNo}`, 'حذف');
        ...
        employees: {
            ...
            async listMonthlyPayroll(month) {
                return listEmployees()
                    ...
                    .map((employee) => buildMonthlyPayrollSummary(employee, listEmployeeCompensations(employee.id), listEmployeeAbsences(employee.id), month));
            },
            async listCumulativePayroll(throughMonth) {
                return listEmployees()
                    .filter((employee) => employee.monthlySalaryIqd && employee.startDate)
                    .map((employee) => buildEmployeeCumulativePayrollSummary(employee, listEmployeeCompensations(employee.id), listEmployeeAbsences(employee.id), throughMonth));
            },
            ...
            async settleMonthlyPayroll(input) {
                const createdEntries = settleMonthlyPayroll(input.month, input.paymentDate, input.paymentMethod, input.createdByEmployeeId, input.createdByEmployeeName, input.employeeIds);
