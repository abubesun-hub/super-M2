...
function resolvePeriod(query) {
    ...
    const preset = ['today', 'month', 'year', 'all', 'custom'].includes(requestedPreset)
        ? requestedPreset
        : 'today';
    let start = null;
    let end = null;
...
function buildHourlyTimeline(invoices) {
...
function buildDailyTimeline(invoices, start, end) {
...
function buildMonthlyTimeline(invoices, start, end) {
...
function getTimelineMeta(period, invoices) {
    if (!invoices.length) {
...
function buildDepartmentBreakdown(invoices, products) {
...
function buildTopProfitProducts(invoices) {
...
dashboardRouter.get('/summary', async (request, response) => {
    const period = resolvePeriod(request.query);
    ...
    const filteredReturns = filteredInvoices.flatMap((invoice) => invoice.returns.filter((saleReturn) => isWithinRange(saleReturn.createdAt, period.start, period.end)));
    ...
    const estimatedProfit = filteredInvoices.reduce((sum, invoice) => sum + invoice.items.reduce((itemsSum, item) => itemsSum + item.lineProfit, 0), 0);
