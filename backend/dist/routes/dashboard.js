...
function buildHourlySales(invoices) {
...
dashboardRouter.get('/summary', async (_request, response) => {
    ...
    const todaysReturns = invoices.flatMap((invoice) => invoice.returns.filter((saleReturn) => isSameDay(new Date(saleReturn.createdAt), now)));
    ...
    const todaysEstimatedProfit = todaysInvoices.reduce((sum, invoice) => sum + invoice.items.reduce((itemsSum, item) => itemsSum + item.lineProfit, 0), 0);
